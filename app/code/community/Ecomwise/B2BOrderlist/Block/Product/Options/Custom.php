<?php 
class Ecomwise_B2bOrderlist_Block_Product_Options_Custom extends Mage_Catalog_Block_Product_View_Options_Abstract {
	 ...
	 * @return string
	 */
	public function getSelectValuesHtml($_option)
	{
		//$_option = $this->getOption();
		$configValue = $this->getProduct()->getPreconfiguredValues()->getData('options/' . $_option->getId());
		$store = $this->getProduct()->getStore();
		$prod_id =$this->getProduct()->getId();
		...
		if ($_option->getType() == Mage_Catalog_Model_Product_Option::OPTION_TYPE_DROP_DOWN
				|| $_option->getType() == Mage_Catalog_Model_Product_Option::OPTION_TYPE_MULTIPLE) {
					...
					if ($_option->getType() == Mage_Catalog_Model_Product_Option::OPTION_TYPE_DROP_DOWN) {
						...
						$select->setName('options['.$prod_id.']['.$_option->getId().']' )
				...
				if ($_option->getType() == Mage_Catalog_Model_Product_Option::OPTION_TYPE_RADIO
						...
						case Mage_Catalog_Model_Product_Option::OPTION_TYPE_RADIO:
							...
							if (!$_option->getIsRequire()) {
								$selectHtml .= '<li><input type="radio" id="options_' . $_option->getId() . '" class="'
										. $class . ' product-custom-option" name="options['. $prod_id .'][' . $_option->getId() . ']"'
					...
					foreach ($_option->getValues() as $_value) {
						...
						. ' product-custom-option"'
								. ($this->getSkipJsReloadPrice() ? '' : ' onclick="opConfig.reloadPrice()"')
								. ' name="options['.$prod_id.'][' . $_option->getId() . ']' . $arraySign . '" id="options_' . $_option->getId()
